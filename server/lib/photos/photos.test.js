...
lab.experiment(TestHelper.getFile(__filename), () => {
  ...
  lab.experiment('get', () => {
    lab.test('gets photo - original', async ({ context }) => {
    ...
    lab.test('gets photo - alternative size', async ({ context }) => {
      const photos = new Photos(photosConfig)
    ...
    lab.test('size not requested', async ({ context }) => {
      const photos = new Photos(photosConfig)
      let errorToCheck
    ...
    lab.test('invalid size requested', async ({ context }) => {
      const photos = new Photos(photosConfig)
      let errorToCheck
  ...
  lab.experiment('deleteObject', () => {
    lab.test('a single photo is deleted', async ({ context }) => {
      const photos = new Photos(photosConfig)
    ...
    lab.test('photo group is deleted (including all alternative sizes)', async ({ context }) => {
      const photos = new Photos(photosConfig)
